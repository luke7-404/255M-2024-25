...
void LCD_Menu::printAuton(lv_color_t color) {
  ...
  for(uint8_t i = 1; i <= 6; i++) {
    ...
    else if(i == 2) lv_label_set_text(btnLabel, "Center GS");
...
void LCD_Menu::auton_eventHandler_STATIC(lv_event_t* e) {
  ...
  if(strcmp(buttonText, "Skills") == 0) {
    checkout = 1;
    checkoutName = "Skills";
  } else if(strcmp(buttonText, "Center GS") == 0) {
    checkout = 2;
    checkoutName = "Center GS";
...
void LCD_Menu::listMotors() {
  ...
  std::vector<pros::Motor> motors = {firstStageIntake, SecondStageIntake, ladyBrownMtr};
